...
```jsx
<div>{ok.value ? <div>yes : <span>no</span>}
```
...
```jsx
<div>{this.ok ? <div>yes : <span>no</span>}
```
...
export default {
  ...
  setup(props, { slots }) {
    return () => [
      ...
      // named slot:
      // <div><slot name="footer" :text="message" />
      h(
        'div',
        slots.footer({
          text: props.message
...
// named
<div>{slots.footer({ text: props.message })}
```
...
// <div><slot name="footer" :text="message" />
<div>{this.$slots.footer({ text: this.message })}
```
